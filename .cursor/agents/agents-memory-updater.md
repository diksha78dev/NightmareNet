...
2. Load the incremental index at `.cursor/hooks/state/continual-learning-index.json` if present.
3. Inspect only transcript files under `~/.cursor/projects/<workspace-slug>/agent-transcripts/` that are new or have newer mtimes than the index (parent `.jsonl` files only; skip `subagents/`).
